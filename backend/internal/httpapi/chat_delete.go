...
func (s *Server) deleteChatMessages(ctx context.Context, userID, conversationID, messageID string) ([]string, string, error) {
	...
	var busy bool
	if err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM chat_turns WHERE conversation_id=$1 AND status IN ('queued','executing','waiting_approval'))`, conversationID).Scan(&busy); err != nil {
		return nil, "", err
	}
	if busy {
