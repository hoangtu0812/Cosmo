...
func (c *Client) observe(ctx context.Context, options Options, started time.Time, usage *Usage, err error) {
	if observer, ok := ctx.Value(observerKey{}).(func(CallObservation)); ok {
		phase, _ := ctx.Value(phaseKey{}).(string)
		if phase == "" {
			phase = "model"
		}
		observer(CallObservation{Phase: phase, Model: c.ResolveModel(options), DurationMS: time.Since(started).Milliseconds(), Usage: usage, Failed: err != nil})
	}
}
