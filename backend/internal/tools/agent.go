	...
	for _, tool := range list {
		for _, action := range actions[tool.ID] {
			...
			if tool.Kind == KindBuiltin && action.Name == "render_html" {
				description += " Dashboard UI requirements: use a clear light header, concise title and only one data-scope notice. Use border-box sizing, responsive filter controls without overflow, consistent 16-24px spacing, and a readable system font. Keep chart labels at least 12px at their actual rendered size. Compute SVG viewBox width from the chart container and redraw on ResizeObserver; avoid shrinking a fixed wide SVG until labels become tiny. Thin time-axis labels to fit (roughly one per 76px), leaving room for first/last labels. Reserve right padding for bar values. Use rounded readable axis ticks, clear series legends, and compact chart headings. On narrow panels stack charts and use two KPI columns where they fit. Tables should have sticky headers, horizontal scrolling, right-aligned numeric columns and tabular numerals. Preserve filters and data when resizing."
				description += " This tool supports real interactive dashboards: inline JavaScript, select/input filters, click handlers, tooltips and drill-down tables all run in the preview. When asked to build a dashboard, call this tool with a working implementation rather than only proposing a layout. Bind controls to one shared filtered dataset and update KPIs, charts and detail rows together, including empty states and reset. Network access and external libraries are unavailable; use inline JavaScript with SVG or canvas. Incomplete attachment data limits coverage, not interactivity: if usable records are available, create the dashboard for those records and visibly label the limited scope. Do not claim full-file accuracy, invent missing records or replace unknown values with zero. If there are no usable records, explain which data is missing instead of fabricating a dashboard."
