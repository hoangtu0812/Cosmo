...
### 2026-09-05 — KB-06a: Snapshot dữ liệu và ghim vào release agent

- Publish KB sao chép passages và vectors của đúng embedding profile vào collection riêng. Manifest đối chiếu ID/version/trạng thái/số chunk của mọi tài liệu và revision cấu hình trước/sau copy; chỉ đổi con trỏ publish khi copy hoàn chỉnh. Lỗi hoặc thay đổi giữa chừng giữ bản đã publish trước đó, dọn collection tạm theo khả năng.
- Migration 28 lưu snapshot theo KB/version, manifest, cấu hình retrieval không chứa API key, số chunk và digest payload. Phiên bản KB cũ không tự được coi là snapshot. Snapshot hiện yêu cầu toàn bộ tài liệu ready và có chunk.
- Agent release chọn live hoặc snapshot. Snapshot ghim bản publish hiện tại của từng KB; thiếu snapshot bị từ chối. Run input và retrieval source lưu ID pin. Chat cũ tiếp tục đúng pin sau khi KB publish bản mới; draft và release live đọc dữ liệu hiện hành.
- Kiểm tra mount/quyền ở admission, worker và sau truy vấn remote. Thu hồi quyền không bị pin vượt qua. Khi một KB timeout, bước kiểm tra quyền vẫn giữ được kết quả hợp lệ của nhánh khác. Snapshot dùng cấu hình embedding đã lưu, key hiện hành; đổi owner/gateway endpoint bị từ chối, không gửi key mới tới endpoint cũ.
...
### 2026-09-06 — KB-05c: Durable ingest/reindex và đổi Live generation nguyên tử

- Migration 34 lưu hàng đợi theo KB. Upload ghi intent/key bản gốc trước gọi MinIO; chỉ trả 202 sau xác nhận lưu tệp và xếp hàng. Crash giữa lưu tệp và đổi trạng thái vẫn có job để nhận lại. Không lưu bytes/API key trong PostgreSQL.
- Worker dùng lease dài hơn deadline tác vụ một phút (deadline theo RAGTimeout, tối đa 30 phút), tối đa ba lần dựng. Mỗi lần có collection riêng; kết quả chậm từ worker cũ không thể đổi con trỏ. Retry không ghi lên collection đang phục vụ. Startup chỉ đánh dấu thất bại với ingestion legacy không có job bền vững.
- Mọi tài liệu thành công, manifest/cấu hình không đổi, người yêu cầu còn quyền và lease còn hợp lệ mới commit con trỏ Live, cấu hình embedding đã dùng, trạng thái tài liệu/job và terminal events cùng transaction. Chỉ mục trước giữ cấu hình embedding cũ cho đến khi đổi thành công. Snapshot publish lấy từ Live generation hiện hành; inspect/delete dùng đúng generation. Xóa tài liệu bị chặn khi KB đang dựng.
