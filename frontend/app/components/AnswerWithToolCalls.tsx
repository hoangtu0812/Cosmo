...
import {ImageSpec, ImageView, imageFromResult} from './ImageView';
 ...
 */
export function AnswerWithToolCalls({calls, children, isStreaming, onOpenChart, onOpenHTML, onOpenImage, messageID}: {
  ...
  onOpenImage?: (image: ImageSpec) => void;
}) {
  ...
  ordered.forEach((call, index) => {
    ...
    parts.push(<VStack key={call.id} gap={2} width="100%"><ToolCallPill call={call} onOpenChart={onOpenChart} onOpenHTML={onOpenHTML} onOpenImage={onOpenImage} /><ToolCallApproval call={call} messageID={messageID} /></VStack>);
    cursor = at;
  });
 ...
 */
function ToolCallPill({call, onOpenChart, onOpenHTML, onOpenImage}: {
  ...
  const image = call.status === 'complete' && call.action === 'generate_image' ? imageFromResult(call.detail) : null;
  const html = call.status === 'complete' && call.action === 'render_html' ? htmlFromResult(call.detail) : null;

  return (
    <VStack gap={2} width="100%">
    {image ? (onOpenImage ? <Button label={image.title} variant="secondary" onClick={() => onOpenImage(image)} /> : <ImageView image={image} />) : null}
    {html ? (onOpenHTML
      ...
      <VStack gap={3} padding={3} width="100%">
        ...
        {call.detail ? (
          <VStack gap={1} width="100%">
            ...
            <Markdown>{image ? 'Ảnh đã tạo' : fence(call.detail)}</Markdown>
          </VStack>
        ) : null}
      </VStack>
    </Collapsible>
